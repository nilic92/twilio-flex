  ...
  async init(flex, manager) {
    ...
    flex.CRMContainer.defaultProps.uriCallback = (task) => {
      if(task && task.attributes.crmid)
        return `https://app.hubspot.com/contacts/${process.env.PORTAL_ID}/contact/${task.attributes.crmid}`
      else 
        return `https://app.hubspot.com/contacts/${process.env.PORTAL_ID}/contacts/list/view/all/`;
    }
  }
}
